 ...
 *   under a human GitHub account, to have at least one GitHub Issue linked
 *   in the PR Development section.
 *
 * How it works
 * - Skips pull requests authored by bots.
 * - Uses GitHub GraphQL with `closingIssuesReferences` and `userLinkedOnly`
 *   to read Development-linked issues.
 * - Fails when no Development-linked issue is present.
