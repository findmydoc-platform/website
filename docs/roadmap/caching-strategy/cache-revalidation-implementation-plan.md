...
| `clinicMedia` | Clinic detail gallery and listing card imagery through the derived clinic thumbnail. | `critical-public` on clinic detail; `aggregated-public` on listings only when the main image or its public listing metadata changes. | The gallery service emits one post-commit clinic event; direct Platform changes to referenced media defer their owning-clinic event until after the response. | Gallery metadata invalidates the bounded clinic path; main-image projection changes add listing and pages-sitemap tags. | Clinic gallery service and clinic-media hook. |
| `clinicGalleryEntries` and `clinicGalleryMedia` | Retained before-and-after schema and records; no active public consumer. | `private-live` while the feature is disabled. | No public cache freshness promise. | No public revalidation. | Controlled maintenance only; reactivation requires a new product decision and legal review. |
