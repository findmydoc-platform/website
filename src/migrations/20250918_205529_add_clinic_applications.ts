...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  CREATE TABLE "clinic_applications" (
  	...
  	"created_artifacts_clinic_id" integer,
  	"created_artifacts_basic_user_id" integer,
  	"created_artifacts_clinic_staff_id" integer,
  	"created_artifacts_processed_at" timestamp(3) with time zone,
  ...
  ALTER TABLE "clinic_applications" ADD CONSTRAINT "clinic_applications_created_artifacts_clinic_id_clinics_id_fk" FOREIGN KEY ("created_artifacts_clinic_id") REFERENCES "public"."clinics"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "clinic_applications" ADD CONSTRAINT "clinic_applications_created_artifacts_basic_user_id_basic_users_id_fk" FOREIGN KEY ("created_artifacts_basic_user_id") REFERENCES "public"."basic_users"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "clinic_applications" ADD CONSTRAINT "clinic_applications_created_artifacts_clinic_staff_id_clinic_staff_id_fk" FOREIGN KEY ("created_artifacts_clinic_staff_id") REFERENCES "public"."clinic_staff"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE INDEX "clinic_applications_created_artifacts_created_artifacts__idx" ON "clinic_applications" USING btree ("created_artifacts_clinic_id");
  CREATE INDEX "clinic_applications_created_artifacts_created_artifact_1_idx" ON "clinic_applications" USING btree ("created_artifacts_basic_user_id");
  CREATE INDEX "clinic_applications_created_artifacts_created_artifact_2_idx" ON "clinic_applications" USING btree ("created_artifacts_clinic_staff_id");
