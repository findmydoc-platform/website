...
import type { ClinicMedia as ClinicMediaDoc } from '@/payload-types'
...
describe('ClinicMedia Collection Access Control', () => {
  ...
  describe('Update/Delete Access (scoped)', () => {
    ...
    test('Other roles cannot update/delete', async () => {
      ...
      expect(
        await ClinicMedia.access!.update!(
          createAccessArgs<AccessArgs<Partial<ClinicMediaDoc>>>(patientReq.user, { payload }),
        ),
      ).toBe(false)
      expect(
      ...
      expect(
        await ClinicMedia.access!.update!(
          createAccessArgs<AccessArgs<Partial<ClinicMediaDoc>>>(anonReq.user, { payload }),
        ),
      ).toBe(false)
      expect(
        await ClinicMedia.access!.delete!(
    ...
    test('Clinic staff without assignment cannot update/delete', async () => {
      ...
      payload.find.mockResolvedValueOnce({ docs: [] })
      expect(
        await ClinicMedia.access!.update!(createAccessArgs<AccessArgs<Partial<ClinicMediaDoc>>>(req.user, { payload })),
      ).toBe(false)
      payload.find.mockResolvedValueOnce({ docs: [] })
      expect(
        await ClinicMedia.access!.delete!(createAccessArgs<AccessArgs<Partial<ClinicMediaDoc>>>(req.user, { payload })),
      ).toBe(false)
    })
  })
})
