...
export function formatAccess(access?: AccessExpectation): string {
  if (!access) return '–'
  ...
  const sanitizedDetails = escapeMarkdownInlineText(access.details)
...
function buildMarkdownTable(rows: MatrixRow[]): string {
  ...
  const lines = rows.map((row) => {
    const cells = [
      `${escapeMarkdownInlineText(row.displayName)} \`(${escapeMarkdownInlineText(row.slug)})\``,
...
function buildNotesSection(rows: MatrixRow[]): string {
  const noteLines = rows
    ...
    .map((row) => `- **${escapeMarkdownInlineText(row.displayName)}**: ${escapeMarkdownInlineText(row.notes ?? '')}`)
