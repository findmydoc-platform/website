...
vercel logs --project findmydoc-portal --scope findmydoc --environment production --since 1h --query "database.runtime.connection_unavailable" --json --limit 100
```
...
| Database runtime availability | `database.runtime.connection_unavailable`; HTTP `503` with `DATABASE_TEMPORARILY_UNAVAILABLE` | Vercel runtime logs; PostHog error tracking; Supabase connection metrics when operator access is approved | P1 when repeated failures block clinic, admin, or submission flows; P2 for an isolated retriable read | Filter by the stable event, group by `databaseFailureKind` and `phase`, and verify the transaction pooler configuration before changing pool values | Technical owner leads triage; release owner joins when the failure starts after a deployment or migration | Do not expose connection strings, database credentials, SQL, original database messages, request bodies, authorization data, or patient data |
