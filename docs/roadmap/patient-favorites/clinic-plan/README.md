...
| `patientClinicInquiries` | Current: clinic, optional patient, contact fields, selected doctor/treatment, status, next step, consent evidence, source path, CRM sync metadata. Future: optional `favoriteClinic`, response/closed timestamps. | Patient-to-clinic contact state. | Platform-owned intake today; patient/clinic scoped access remains future. | Submitted timestamps today; response lifecycle remains future. | Partial source exists for clinic profile contact UI. |
