...
function buildMissingLinkedIssueComment() {
  return [
    ...
    '',
    'Preferred `Closes` links in the pull request body:',
    '- same repository: `Closes #123`',
    '- trusted cross-repository: `Closes findmydoc-platform/management#123`',
    '- repeat `Closes` once per Issue',
    '',
    'Accepted fallback links:',
    "- a manual link in GitHub's `Development` section",
    '- a standard GitHub closing reference on a pull request that targets the default branch',
    '- a trusted cross-repository closing reference to `findmydoc-platform/management`',
