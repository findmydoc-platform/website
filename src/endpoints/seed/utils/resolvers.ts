...
export function createStableIdResolvers(payload: Payload): StableIdResolvers {
  ...
  const resolveStableIdById = async (collection: CollectionSlug, id: string | number) => {
    ...
    try {
      ...
      const isNotFound =
        err.status === 404 || err.statusCode === 404 || err.name === 'NotFound' || message.includes('not found')
      if (!isNotFound) {
        payload.logger.error(error, `Error resolving stableId for ${collection} (id: ${String(id)})`)
        throw error
      }
      doc = undefined
    }
