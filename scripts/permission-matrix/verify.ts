...
async function main() {
  ...
  try {
    ...
    const sourceCollectionCount = collectionSlugs.size

    for (const slug of managedPluginCollectionSlugs) {
      collectionSlugs.add(slug)
      collectionFileMap.set(slug, 'generated by src/security/generatedCollectionAccess.ts')
    }

    console.log(`✅ Found ${sourceCollectionCount} collection definitions in src/collections/`)
    console.log(`✅ Found ${managedPluginCollectionSlugs.length} managed plugin collections`)
