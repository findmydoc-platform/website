...
1.  **Strict Separation**: Build "Dumb" (Presentational) components that rely _only_ on props.
...
4.  **Parent-Controlled State (Controlled Components)**: Interactive inputs/controls must be **parent-controlled** via props (e.g., `value` + `onValueChange`, `checked` + `onCheckedChange`). Do not keep internal state for the primary control value (internal state is allowed only for ephemeral UI like open/closed, or compound-component coordination). References: ADR 011 (`docs/adrs/011-adr-parent-controlled-components.md`) and ADR index (`docs/adrs/README.md`).
...
- If a sub-component needs to be hidden, the consumer should simply _not render it_, rather than passing a `hidden` prop.
...
- **Atomic Utilities**: Use the CSS `@utility` directive _only_ for small, generic composition shortcuts (e.g., `@utility flex-center`) that are not tied to specific business components.
...
- **The "Design System" Rule (Forbidden)**: Do **NOT** use arbitrary values for primary styling tokens.
  - _Forbidden:_ `text-[#123456]`, `font-[16px]`, `gap-[13px]`.
  - _Action:_ These must be defined as variables in `@theme` to ensure consistency.
- **The "External Constraint" Exception (Allowed)**: You may use arbitrary values _only_ for pixel-perfect positioning forced by external libraries or assets (e.g., `top-[64px]` for a sticky header, `z-[100]` for overlays).
- **Justification Requirement**: If you use `[...]`, you must output a comment in the code explaining _why_ the theme variables were insufficient.
...
- **Mobile-First Strictness**: Write base classes for **mobile** first. Use breakpoints (`sm:`, `md:`, `lg:`) _only_ to override for larger screens.
  - _Bad:_ `sm:text-center` (implies mobile styling is broken or accidental).
  - _Good:_ `text-center md:text-left`.
