...
Status: planned. This document prepares implementation; it does not change runtime behavior.

The first implementation slice establishes the server-to-server integration required by the standalone Clinic
Dashboard. It does not add browser access to Payload, Payload CORS origins, a Dashboard database, service-role
credentials, public cache behavior, or clinic login UI to the website.
...
The initial focused endpoint is conceptually `GET /api/clinic-dashboard/bootstrap`. Its exact registration must follow
Payload endpoint conventions when implemented, but its stable semantic contract is fixed here.
...
The Dashboard owns its same-origin routes. The paired plan defines their local file placement; this repository owns only
the upstream Payload behavior they require.
...
1. Add focused contract tests for clinic Bearer authentication, principal resolution, approval, clinic assignment, and
   denied states.
2. Implement and register the bootstrap DTO and endpoint without changing Payload CORS.
3. Add endpoint permission and response-projection tests, including PII and raw-document leak assertions.
4. Integrate the Dashboard server-only client and same-origin routes from the paired plan.
5. Validate local and trusted preview authentication against Staging before configuring the exact production callback.
6. Add later capability endpoints one bounded user operation at a time.

The existing Payload API follow-up owns the website portion of steps 1–3. Dashboard wiring remains a separate runtime
change in the Dashboard repository.

## Acceptance Evidence for the Runtime Follow-up
