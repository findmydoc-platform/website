...
export function ClinicDetailConceptB({ data, className }: ClinicDetailConceptProps) {
  return (
    <main className={cn('bg-background text-foreground', className)}>
      <Container className="space-y-12 py-12 md:space-y-14">
        <section className="grid gap-6 lg:grid-cols-12 lg:items-stretch">
          <Card className="overflow-hidden border-primary/30 lg:col-span-8">
            <CardContent className="space-y-5 p-6 md:p-8">
              <p className="text-xs tracking-[0.2em] text-primary uppercase">Transparency First</p>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
          <ClinicTrustMetrics trust={data.trust} title="Trust Signals" />
          <TreatmentsPricePanel treatments={data.treatments} title="Transparent Pricing" />
        </section>
