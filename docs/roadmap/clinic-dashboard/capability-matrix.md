...
| Dashboard API | Payload exposes the private-live bootstrap plus persistent clinic-profile snapshot, draft save/discard, and atomic publish contracts. | The Dashboard integrates these focused contracts server-side. Payload derives the clinic from the current principal; browser CORS remains unnecessary. |
...
| Visible action or state | Current capability and allowed behavior | Status | Owner or dependency | Cache impact |
| --- | --- | --- | --- | --- |
| List, filter, refresh, and paginate own-clinic reviews | Clinic staff can read approved own-clinic reviews, including removed and withdrawn state, through the sanitized current contract. Public measures are `none`, `context`, `redaction`, `placeholder`, and `removed`; withdrawal is separate. Raw unsafe text, internal reasons, patient relations, and named actors remain hidden. The Dashboard still owns typed filters, pagination, refresh behavior, and its purpose-limited BFF projection. | `existing` website contract; Dashboard UI pending | website#1642; clinic-dashboard#106 | `private-live` management read |
...
| Show review and response history | `reviews` uses unlimited native versions, but raw review versions are platform-only. Clinic staff use the own-clinic sanitized `publication-history` endpoint so earlier unsafe text cannot leak. `reviewResponses` native versions remain own-clinic readable. Restore is disabled. | `existing` website contract; Dashboard UI pending | website#1642; clinic-dashboard#106 | `private-live`, `no-store` |
| Appeal or flag a review | `reviewAppeals` stores one immutable appeal per approved review with submitted, under-review, upheld, and dismissed states. Only platform staff decide. The decision remains separate from public review moderation: `upheld` changes neither the review nor its clinic response and requires an explicit public measure recorded after submission. | `existing` website contract; Dashboard UI pending | #1529; clinic-dashboard follow-up | Every appeal state is `private-live`; a separate review or response moderation change owns any public invalidation |
...
| Visible action or state | Current capability and allowed behavior | Status | Owner or dependency | Cache impact |
| --- | --- | --- | --- | --- |
| Edit clinic name, description, address, phone, and coordinates | Fields and own-clinic update rules exist in `clinics`; approved writes already revalidate public clinic surfaces. The main image is managed through the profile gallery rather than as an independent thumbnail field. | `existing` | #1524 for BFF/Payload access | `public-cached` |
| Change the main image or a before/after gallery item | The first ordered profile-gallery image derives the thumbnail and public main image. Before/after collections and relationships remain retained, hidden, and fail-closed. | `existing` for main image; `disabled` for before/after | website#1619; #1576 requires a new decision and legal review before before/after reactivation | Main image `public-cached`; before/after `private-live` |
| Add or reorder a generic clinic gallery | Website #1619 provides the tenant-derived upload, ordered save, discard, cleanup, and revision-conflict contracts for up to twelve profile images. | `existing` website contract; Dashboard UI pending | website#1619; clinic-dashboard#100 | Detail `public-cached`; listing and sitemap invalidation only when the first image changes |
...
| `clinicMedia`, `clinicGalleryMedia`, `clinicGalleryEntries` | Ordered clinic profile gallery plus retained before/after schema and records | Profile media belongs to one clinic; `clinics.profileGallery[0]` derives `thumbnail` | `clinicMedia` reads remain own-clinic/Platform-only; Clinic Staff writes use the custom revision-protected gallery endpoints and direct collection writes are Platform-only; public files require published status and an approved clinic; both before/after collections remain hidden and fail-closed | Gallery metadata invalidates the owning clinic detail; a changed first image or its public listing metadata additionally invalidates listing and sitemap surfaces | Profile gallery is implemented by website#1619; before/after remains `disabled` by #1576 |
...
1. Direct referenced `clinicMedia` changes invalidate the owning clinic detail and include listing/sitemap surfaces when the media is the derived thumbnail. Direct `doctorMedia` dependency invalidation remains deferred to [website#1468](https://github.com/findmydoc-platform/website/issues/1468).
